...
# 1С переименовывал позиции — старое имя → каноническое (применяется до канона)
SKU_RENAMES: dict[str, str] = {
    "Конфеты Курага в МШ 90гр":    "Драже Курага в МШ 90гр",
    "Конфеты Чернослив в ТШ 90гр": "Драже Чернослив в ТШ 90гр",
}
...
MANUAL_OVERRIDES: dict[str, tuple[str, str]] = {
    ...
    # Новинки драже 2026 — линейка мини-форматов в дойпаках (6 видов).
    # Пришли из 1С с «Без подкатегории»; мода их не вылечит, канон задаём явно
    "Драже Кешью в МШ 90г": ("Драже", "Драже дойпак"),
    "Драже Клубника в МШ 80г.": ("Драже", "Драже дойпак"),
    "Драже Фундук в МШ 90г": ("Драже", "Драже дойпак"),
    "Драже Чернослив в ТШ 90гр": ("Драже", "Драже дойпак"),
    "Драже Миндаль в ТШ 90г": ("Драже", "Драже дойпак"),
    "Драже Курага в МШ 90гр": ("Драже", "Драже дойпак"),
...
def main() -> None:
    ...
    # --- 0. Переименования SKU (1С менял имена позиций) ---
    ren_mask = df["Номенклатура"].isin(SKU_RENAMES)
    if ren_mask.any():
        print(f"  переименования SKU: {int(ren_mask.sum())} строк")
        df.loc[ren_mask, "Номенклатура"] = df.loc[ren_mask, "Номенклатура"].map(SKU_RENAMES)
